...
2. **Install the extra once per venv:**
   ...
   them; `/realsense/status` then reports `installed: false` with the fix.
...
3. **Enable it** in `src/settings/realsense.yaml` (`enabled: true`; pin
   `serial` if more than one RealSense is attached) and restart the service.
4. Open the panel: a **Depth Camera** card appears in the right column. It
...
|---|---|---|---|
| GET | `/realsense/status` | open | enumeration + pipeline state (`configured`, `installed`, `state`, `devices[]`, `device`, `streams`, `fps_measured`, `warnings[]`, `reason`) |
| POST | `/realsense/start` | login | opens the pipeline; **503** `realsense_unavailable` (disabled / driver missing / no camera), **502** `realsense_error` (librealsense refused the profiles — usually a USB 2 link) |
| POST | `/realsense/stop` | login | releases the device |
| GET | `/realsense/snapshot.jpg?stream=color\|depth` | login | one JPEG (colour, or colourised depth); starts on demand |
| GET | `/realsense/depth.png` | login | raw 16-bit depth map, lossless; `X-Depth-Scale-M` header converts to metres |
| GET | `/realsense/stream.mjpg?stream=color\|depth&fps=10` | login | `multipart/x-mixed-replace` MJPEG for an `<img>`; paced server-side |
| GET | `/realsense/depth?x=&y=&window=5` | open | `{distance_m, point_m:[X,Y,Z], valid_samples, …}`; **409** when stopped — never starts the camera |
| GET | `/realsense/intrinsics` | open | pinhole intrinsics per stream + `depth_scale_m` + `aligned_to` |
...
with codes `realsense_not_configured` (404), `realsense_unavailable` (503),
`realsense_not_streaming` (409), `realsense_error` (502), `bad_request` (400).
...
- **A daemon capture thread owns the pipeline.** `wait_for_frames` blocks,
  ...
  camera lost and release it; `POST /realsense/start` brings it back.
- **Process-wide, not per-connection.** The camera outlives `/connect` /
  `/disconnect`: operators want to see the bench before the arm is up.
  `status_builder` reads it through `realsense_camera.shared_camera()` so
  `/status` and `/realsense/status` cannot disagree.
- **Injectable backend.** `RealSenseCamera(config, rs_module=…, np_module=…)`
...
| stream freezes, then `state: error` | cable yanked / power dip | re-seat, `POST /realsense/start` |
