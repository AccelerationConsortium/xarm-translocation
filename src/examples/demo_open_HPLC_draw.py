#!/usr/bin/env python3
"""
HPLC Drawer Opening Demo Script

This script demonstrates a complete sequence for opening an HPLC drawer:
1. Linear motor to home position
2. Robot joints to home position
3. Joint movement to uplc_draw_home
4. Joint movement to uplc_draw_open_max
5. Linear movement to uplc_draw_open_min
6. Linear movement to uplc_draw_open_close
7. Linear movement back to uplc_draw_open_min
8. Linear movement to uplc_draw_open_max
9. Joint movement back to robot home

Usage:
    python demo_open_HPLC_draw.py --real        # Use real hardware
    python demo_open_HPLC_draw.py --simulate    # Use simulation mode
...
def get_speed_config():
    """
    Define speed configurations for each step of the demo.
    
    Returns:
        dict: Speed configuration for each step
    """
    return {
        'linear_home': {
            'track_speed': 100,  # mm/s - Fast return to home
            'description': 'Fast track movement to home'
        },
        'robot_home': {
            'joint_speed': 20,   # °/s - Moderate speed for safety
            'description': 'Moderate speed joint movement to home'
        },
        'uplc_draw_home': {
            'joint_speed': 20,   # °/s - Slower for precision positioning
            'description': 'Precise positioning to UPLC draw area'
        },
        'uplc_draw_open_max': {
            'joint_speed': 10,   # °/s - Slow for approach
            'description': 'Careful approach to drawer'
        },
        'uplc_draw_open_min': {
            'tcp_speed': 50,     # mm/s - Very slow linear movement for fine positioning
            'description': 'Precise linear positioning near drawer'
        },
        'uplc_draw_open_close': {
            'tcp_speed': 20,     # mm/s - Very slow linear movement for drawer interaction
            'description': 'Very slow linear movement for drawer closing'
        },
        'uplc_draw_return_min': {
            'tcp_speed': 20,     # mm/s - Slow linear movement backing away
            'description': 'Slow linear retraction from closed drawer'
        },
        'uplc_draw_return_max': {
            'tcp_speed': 50,     # mm/s - Moderate linear movement moving away
            'description': 'Moderate linear movement away from drawer'
        },
        'final_home': {
            'joint_speed': 20,   # °/s - Faster return to home
            'description': 'Return to home position'
        }
    }


def demo_hplc_drawer_opening(controller, auto_confirm=False, custom_speeds=None):
    """
    Execute the complete HPLC drawer opening sequence.
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("🔬 HPLC DRAWER OPENING DEMO")
        print("=" * 60)
        ...
        print("4. Joint movement → uplc_draw_open_max")
        print("5. Linear movement → uplc_draw_open_min")
        print("6. Linear movement → uplc_draw_open_close")
        print("7. Linear movement → uplc_draw_open_min")
        print("8. Linear movement → uplc_draw_open_max")
        ...
        # Step 4: Joint movement to uplc_draw_open_max
        joint_speed = speeds['uplc_draw_open_max']['joint_speed']
        if not move_with_confirmation(
            controller,
            lambda: controller.move_to_named_location('uplc_draw_open_max', speed=joint_speed),
            "Step 4: Joint movement to uplc_draw_open_max position",
            auto_confirm,
            speeds['uplc_draw_open_max']
        ):
            return False
    
        # Step 5: Linear movement to uplc_draw_open_min
        ...
        # Step 6: Linear movement to uplc_draw_open_close
        ...
        # Step 7: Linear movement back to uplc_draw_open_min
        ...
        # Step 8: Linear movement to uplc_draw_open_max
        ...
        print("\n🎉 HPLC Drawer Opening Demo completed successfully!")
        print("=" * 60)
        return True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='HPLC Drawer Opening Demo')
    ...
    parser.add_argument('--auto', action='store_true', 
                       help='Auto-confirm all movements (no user prompts)')
    parser.add_argument('--slow', action='store_true',
                       help='Use slower speeds for all movements (safer)')
    parser.add_argument('--fast', action='store_true',
                       help='Use faster speeds for all movements (quicker)')
    parser.add_argument('--speed-multiplier', type=float, default=1.0,
                       help='Multiply all speeds by this factor (default: 1.0)')
    ...
    if args.slow:
        ...
        speed_description = "Fast (1.5x)"
    else:
        speed_multiplier = args.speed_multiplier
        if speed_multiplier != 1.0:
            speed_description = f"Custom ({speed_multiplier}x)"
    
    # Apply speed multiplier if not default
    if speed_multiplier != 1.0:
    ...
    try:
        ...
        # Run the demo
        success = demo_hplc_drawer_opening(controller, auto_confirm, custom_speeds)
