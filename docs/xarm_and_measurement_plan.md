...
- ✅ **Gripper verification fails *closed* — decided & shipped (2026-07-21):** when the hardware position can't be read, `_verify_gripper` now reports the GRASP/POSITION verification as **failed** (was: warn + return `True`) — "couldn't check" must never pass for "verified held". `NONE` intent still skips the read entirely. Unit-tested (`test/test_gripper_verification.py`, unreadable-position cases for both intents).
- ✅ **STRICT is now the boot default (2026-07-21):** `graph_mode` boots `STRICT` when `motion_graph.yaml` loads (`src/core/xarm_controller.py`), so off-whitelist moves are rejected, not just advised. Soft-fail to `OFF` on missing/invalid YAML is retained (arm stays drivable with raw moves while iterating on the graph); runtime downgrade stays available via `/control/graph/mode`. On connect at an arbitrary pose the arm is off-grid under STRICT — the recovery path is `recover_to` (a claim-gated re-pin declaration, no motion, works in any mode) → graph moves. Boot default pinned by `test/test_motion_graph_phase2.py::test_boot_mode_is_strict_when_graph_loads`. Re-wiring the commented-out stations remains ahead.
