...
In `ac-organic-lab/api/app/control.py:455-500`, add a size-bounded, secret-stripped `node_id`/`mode` (or bounded `request_body`) to the `control_action` payload so every executed `graph/move_to`/`recover_to` is self-describing. **Contracts:** STATUS_SPEC §8 (redact; ≤500 chars). **Risk:** low; audit-only. **Reversible:** revert the field. *(Verified still absent as of 2026-07-19.)*
...
### Step 4 — Flip xArm to v1.1 + surface graph controls — ◑ PARTIAL
✅ Registry flipped to `protocol: "1.1"` (claim dance on); `do_not_call_connect` retained. ▢ Remaining: (a) the dashboard tile still has no `graph.*` buttons — add them through the audited `/api/equipment/xarm_translocation/control/graph/*` passthrough, gated by the `CONTROL_PASSWORD` lock chip; (b) reconcile the catalog (4 SkillDefs: `graph.{move_to,recover_to,record,mode}`) with the device's larger verb set (`travel_to`, `gripper`, `edge*`, `node`) and its advertised `allowed_actions`; (c) mirror the SkillDefs in `typed_clients/robot_arm.py`. **Files:** `ac-organic-lab/web/src/components/RobotArmTile.tsx`, `skill_catalog/robot_arm.py`, `typed_clients/robot_arm.py`. **Risk:** medium (live control) — verify tokenless `/control/graph/*` → 423 first. **Reversible:** remove buttons; flip `protocol` back.
