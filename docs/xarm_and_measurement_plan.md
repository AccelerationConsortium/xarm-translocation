...
- ▢ **One-way UPLC-drawer trap** (found this session): three drawer nodes (`uplc_draw_open_max`/`_min`/`_close`) are reachable from home but have **no path back** — the return edge `uplc_draw_open_max → uplc_draw_home` is missing. Harmless in the current ADVISORY default (off-whitelist moves warn + execute), but STRICT would strand the arm in the open drawer. **Fix:** add the return edge.
- ▢ **Reverse-reachability guard:** the connectivity guard only checks forward (home → node); the safety-critical direction is node → home ("can the arm always retreat?") — exactly what would have caught the drawer trap. **Fix:** extend the guard to assert every connected node can reach `robot_home`.
